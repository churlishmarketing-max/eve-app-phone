...
export async function buildContextPack(
  ...
  return (await buildPackLines(surface, incomingMessage, conversationId, includeHistory, desk, deskRefusal, untrusted))
    .map((l) => l.text)
    .join("\n");
}
...
export async function buildPackLines(
  ...
  if (recall.length) {
    ...
    for (const r of recall) lines.push(...L("recall", `  - [${r.kind} · ${r.created_at.slice(0, 10)}] ${r.content}`));
  }
  lines.push(
    ...L(
      "honesty",
